...
// These types match the admin portal's orderService types
interface QRCode {
  code: string
  type: string
  day?: number
...
const AdminTicketDocument = forwardRef<HTMLDivElement, TicketProps>(({ order, event }, ref) => {
    ...
    <div ref={ref} style={{ fontFamily: 'sans-serif', width: '600px', backgroundColor: '#fff' }}>
      ...
      {qrCodes.map((qr, i) => {
        ...
        const cardTitle = isMeal
          ...
          : isTransport ? 'TRANSPORTATION TICKET'
          : isAccommodation ? 'ACCOMMODATION TICKET'
          : qr.type.toUpperCase()
...
AdminTicketDocument.displayName = 'AdminTicketDocument'
export default AdminTicketDocument