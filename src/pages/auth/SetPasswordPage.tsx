...
/**
 * Unified set-password page — used for BOTH new-admin onboarding and forgot-password
 * reset (opened from the emailed /set-password?token=... link). The token in the body
 * carries the context on the backend, so a single "create a new password" form serves both.
 */
      ...
      try {
        ...
        const message =
          (err as { response?: { data?: { message?: string } } })
            ?.response?.data?.message ?? 'Could not set your password. The link may have expired.'
        toast.error(message)
      } finally {
        setSubmitting(false)
      }
      ...
      <AuthLayout>
        <div className="text-center">
          ...
          <p className="text-[15px] text-gray-500 mb-6">
            This link is missing or has expired. Please request a new one or ask an admin to resend your invite.
          </p>
