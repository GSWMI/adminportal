...
import { exportTransportTicketsClientSide } from '../../../services/exportService'
...
export default function TransportTicketsPage() {
  ...
  const handleExport = async () => {
    setExporting(true)
    try { exportTransportTicketsClientSide(orders, event?.name) } catch { toast.error('Export failed') }
  ...
  const getPickupLocation = (order: OrderData) => {
    // First try qrCodes which has pickupLocation directly
    const qr = order.qrCodes?.find((q) => q.type === 'transport')
    if (qr?.pickupLocation) return qr.pickupLocation
    // Fallback to event transport list
    const transportId = order.transportId
